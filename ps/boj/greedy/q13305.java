...
public class q13305 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < cityLength.length; i++) {
            cityLength[i] = Integer.parseInt(stringTokenizer.nextToken());
        }

        stringTokenizer = new StringTokenizer(bufferedReader.readLine(), " ");
        int[] fuelPrice = new int[city];
        for (int i = 0; i < fuelPrice.length; i++) {
            fuelPrice[i] = Integer.parseInt(stringTokenizer.nextToken());
        }

        long price = 0;
        int fuelCity = 0;
        for (int i = 0; i < city - 1; i++) {
            if (fuelPrice[fuelCity] > fuelPrice[i]) {
                fuelCity = i;
            }
            price += (long) fuelPrice[fuelCity] * cityLength[i];
        }
